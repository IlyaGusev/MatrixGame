...
fn main() {
    ...
    let dim = 16;
    for (label, k) in [("(1,1)", 1*dim+1), ("(1,2)", 1*dim+2), ("(2,1)", 2*dim+1), ("(2,2)", 2*dim+2)] {
        if un[k] < 0 { println!("{}: empty", label); continue; }
        ...
        let base_name = if base_id < strings.arrays_count() { strings.get_as_wstr(base_id) } else { "??".to_string() };
        println!("{}: base={} (id {}), {} overlays:", label, base_name, base_id, (bot.len()-1)/2);
        ...
        while bi + 1 < bot.len() {
            ...
            println!("  overlay: {} (ids={}, mask={}, mask_bytes={})", overlay_name, ids, ibm, mask_size);
        }
    }
}
