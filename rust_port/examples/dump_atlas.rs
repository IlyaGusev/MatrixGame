            ...
            if lp {
                for u in 0..(tbs / 2 - 2) {
                    copy_col(
                        ...
                        xx + u,
                        yy + if tp { u } else { 0 },
                        tbs - if tp { u } else { 0 } - if bp { u } else { 0 },
                        xx - 1,
                        yy + if tp { u } else { 0 },
                    );
                }
            ...
            if tp {
                for u in 0..(tbs / 2 - 2) {
                    copy_row(
                        ...
                        xx + if lp { u } else { 0 },
                        yy + u,
                        tbs - if lp { u } else { 0 } - if rp { u } else { 0 },
                        xx + if lp { u } else { 0 },
                        yy - 1,
                    );
                }
        ...
        for px in 0..w {
            ...
            atlas.put_pixel(
                ...
                image::Rgba([
                    ...
                    ((d[2] as u16 * inv + s[2] as u16 * a) / 255) as u8,
                    255,
                ]),
            );
        }
...
fn copy_col(img: &mut image::RgbaImage, dx: i32, dy: i32, h: i32, sx: i32, sy: i32) {
    ...
    for i in 0..h {
        ...
        img.put_pixel(
            dx.clamp(0, w - 1) as u32,
            (dy + i).clamp(0, ih - 1) as u32,
            p,
        );
    }
}
...
fn copy_row(img: &mut image::RgbaImage, dx: i32, dy: i32, w: i32, sx: i32, sy: i32) {
    ...
    for i in 0..w {
        ...
        img.put_pixel(
            (dx + i).clamp(0, iw - 1) as u32,
            dy.clamp(0, ih - 1) as u32,
            p,
        );
    }
}
