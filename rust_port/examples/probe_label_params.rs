...
fn main() {
    ...
    for i in 0..names.arrays_count() {
        ...
        if !["cobuild", "cocan", "chasl", "weapl", "buildl", "it_label1", "it_label2"]
        ...
        let Some(child_names) = stor.get_buf(&rec, "2") else { continue; };
        let Some(child_recs) = stor.get_buf(&rec, "3") else { continue; };
        println!("\n--- {kind} '{elem_name}' (children: {}) ---", child_names.arrays_count());
        for j in 0..child_names.arrays_count() {
            ...
            let Some(lbl_names) = stor.get_buf(&cr, "2") else { continue; };
            let Some(lbl_recs) = stor.get_buf(&cr, "3") else { continue; };
